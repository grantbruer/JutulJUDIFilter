...
function build_source_receiver_geometry(; params...)
    ...
    if setup_type == :surface
        ...
        srcGeometry = (;xloc=xsrc, yloc=ysrc, zloc=zsrc)
        ...
        recGeometry = (;xloc=xrec, yloc=yrec, zloc=zrec)
...
function plot_points_of_interest!(ax; params, idx_wb, idx_unconformity)
    ...
    lines!(ax, x, y; linewidth=6, color=BLACK,linecap=:square)
    lines!(ax, x, y; linewidth=3, color=GREEN,linecap=:square)
    le_injection = ("Injection range", [MarkerElement(color = BLACK, marker = :rect, markersize = 20),MarkerElement(color = GREEN, marker = :rect, markersize = 16)])
    ...
    for i in 1:nsrc
        xsrc[i] = srcGeometry.xloc[i] ./ 1e3 .- mesh_3d.origin[1]
        ysrc[i] = srcGeometry.zloc[i] ./ 1e3 .- mesh_3d.origin[end]
    end
    ...
    nrec = length(recGeometry.xloc)
    xrec = recGeometry.xloc ./ 1e3 .- mesh_3d.origin[1]
    yrec = recGeometry.zloc ./ 1e3 .- mesh_3d.origin[end]
...
function my_anim_reservoir_plotter(idx, getter, states;
    ...
    fig, ax, hm = plot_heatmap_from_grid(data, grid; colormap, colorrange, fix_colorrange=false, kwargs...)
...
function my_anim_reservoir_plotter_overlay(idx, getter, states;
    post_plot=(fig, ax)->nothing,
    colormap=parula,
    colorrange=nothing,
    grid,
    ...
    data_combo = @lift(getter(states[$idx]))
    data = @lift($data_combo[1])
    overlay = @lift($data_combo[2])
    function get_time_string(state)
        if haskey(state, :time_str)
            return state[:time_str]
        end
    ...
    colorrange = @lift(extrema($overlay))
    hm2 = plot_heatmap_from_grid!(ax, overlay, grid; colormap=:grays, colorrange, alpha=0.4, fix_colorrange=false, make_heatmap=true)
    if overlay_grid
        translate!(hm, 0, 0, -100)
...
function plot_plume_with_permeability(states, K, save_dir_root; params, extra_data)
    n_3d = Tuple(params["transition"]["n"])
    d_3d = Tuple(params["transition"]["d"])
    dt = params["transition"]["dt"]
    ...
    # extras_base = (; params, extra_data..., grid=mesh_3d_cutoff)
    extras_base = (; params, grid=mesh_3d_cutoff)
    framerate = 2


    # # Plot saturation with permeability, continuous.
    # delete_colorbar = true
    # get_simple = function(state)
    #     sat = reshape(ifelse.(state[:Saturation] .== 0, -1, state[:Saturation]), n_3d[1], n_3d[end])[:, cutoff_idx:end]
    #     perm = reshape(K, n_3d[1], n_3d[end])[:, cutoff_idx:end]
    #     return sat, perm
    # end
    # extras = (; post_plot, make_colorbar=false,  colorrange=(0,1), lowclip=:transparent, make_heatmap=true, extras_base...)
    # # extras = (; post_plot, colorrange = (0, 1), make_colorbar=false, extras_base...)
    # file_path = joinpath(save_dir, "saturation-continuous.mp4")
    # plot_anim(states, get_simple, my_anim_reservoir_plotter_overlay, file_path; extras, framerate)
    # return

    let
        get_simple = function(state)
            sat = reshape(ifelse.(state[:Saturation] .== 0, -1, state[:Saturation]), n_3d[1], n_3d[end])
            return sat
        end
        levels = range(0, 1, length = 11)
        extras = (; post_plot, levels, make_colorbar=false, extendlow = :transparent, extras_base..., grid=mesh_3d)

        # Plot saturation.
        delete_colorbar = true
        file_path = joinpath(save_dir, "saturation_full.mp4")
        plot_anim(states, get_simple, my_anim_reservoir_plotter, file_path; extras, framerate)

        # Plot saturation colorbar vertical.
        delete_colorbar = false
        extras = (; extras..., make_colorbar=true)
        fig = my_anim_reservoir_plotter(1, get_simple, states; extras...)
        axis_setup(fig.content[1]; cb_label = nothing, delete_colorbar = false)
        delete!(fig.content[1])
        delete!(fig.content[2])
        cb = fig.content[1]
        ...
        cb.ticks = levels
        colsize!(fig.layout, 1, 0.0)
        file_path = joinpath(save_dir, "saturation_colorbar_full_vertical.png")
        resize_to_layout!(fig)
        save(file_path, fig)
    end
    return
    ...
    let
        get_simple_perm = function(state)
            sat = reshape(ifelse.(state[:Saturation] .== 0, -1, state[:Saturation]), n_3d[1], n_3d[end])[:, cutoff_idx:end]
            perm = reshape(K, n_3d[1], n_3d[end])[:, cutoff_idx:end]
            return sat, perm
        end
        ...
        # Plot saturation with permeability.
        delete_colorbar = true
        file_path = joinpath(save_dir, "saturation_perm.mp4")
        plot_anim(states, get_simple_perm, my_anim_reservoir_plotter_overlay, file_path; extras, framerate)
        ...
        cb.ticks = levels
        colsize!(fig.layout, 1, 0.0)
        file_path = joinpath(save_dir, "saturation_colorbar_vertical.png")
        resize_to_layout!(fig)
        save(file_path, fig)
    end
    ...
    # Plot saturation error colorbar.
    delete_colorbar = false
    extras = (; extras_base..., make_colorbar=true, colorrange = (-1, 1), colormap = :balance, make_heatmap = true)
    fig = my_anim_reservoir_plotter(1, get_simple, states; extras...)
    axis_setup(fig.content[1]; cb_label = nothing, delete_colorbar = false)
    delete!(fig.content[1])
    delete!(fig.content[2])
    ...
    # cb.ticks = levels
    colsize!(fig.layout, 1, 0.0)
    file_path = joinpath(save_dir, "saturation_error_colorbar_vertical.png")
    resize_to_layout!(fig)
    save(file_path, fig)
    return
...
function plot_rtm_data(states, save_dir_root; params, extra_data)
    ...
    # Display mesh in kilometers.
    origin = (params["transition"]["injection"]["loc"][1] / 1000 - d[1]/2000, 0 - d[end]/2000)
    grid = CartesianMesh(n, d .* n ./ 1e3; origin)
    ...
    for colormap in [:balance]
        extras = (; post_plot, params, grid, colormap, divergent=true, make_heatmap=true, overlay_grid=(colormap != :grays))
        framerate = 2
        ...
        fig, ax, hm = plot_heatmap_from_grid(get_simple(states[1]), grid; colormap, make_divergent=true, make_heatmap=true)
        if extras.overlay_grid
            translate!(hm, 0, 0, -100)
        end
        post_plot(fig, ax)
        save(file_path, fig)
        ...
        # Plot colorbar.
        fig = my_anim_reservoir_plotter(Observable(1), get_simple, states; extras..., make_colorbar=true)
        axis_setup(fig.content[1]; cb_label = colorbar_label, delete_colorbar = false)
        delete!(fig.content[1])
        delete!(fig.content[2])
        cb = fig.content[1]
        cb.vertical = true
        colsize!(fig.layout, 1, 0.0)
        file_path = joinpath(save_dir, "rtms_baseline_colorbar_vertical_$colormap.png")
        resize_to_layout!(fig)
        save(file_path, fig)
        ...
        let extras = (; extras..., colorrange=(-160, 160))
            ...
            cb.vertical = true
            colsize!(fig.layout, 1, 0.0)
            file_path = joinpath(save_dir, "rtm_offset_colorbar_vertical_cb_$colormap.png")
            resize_to_layout!(fig)
            save(file_path, fig)
        end
        ...
        let extras = (; extras..., colorrange=(-700, 700))
            ...
            cb.vertical = true
            colsize!(fig.layout, 1, 0.0)
            file_path = joinpath(save_dir, "rtms_plain_colorbar_vertical_cb_$colormap.png")
            resize_to_layout!(fig)
            save(file_path, fig)
        end
    end
...
save_dir = joinpath("figs", "ground_truth2")
mkpath(save_dir)

if ! (@isdefined states_plume) || isnothing(states_plume)
    states_plume, extra_data = read_ground_truth_plume_all(params, job_dir)
end
...
println("Finished early"); asdf(0)

if ! (@isdefined states_rtm) || isnothing(states_rtm)
...
# Plot plume with permeability.
plot_plume_with_permeability(states_plume, K, save_dir; params, extra_data)
println("Finished early"); asdf(0)

# Plot plume.
plot_plume_data(states_plume, save_dir; params, extra_data)
println("Finished early"); asdf(0)
