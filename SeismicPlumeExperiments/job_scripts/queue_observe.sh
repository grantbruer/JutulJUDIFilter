...
if [ "$num_jobs" -eq 0 ]; then
    ...
    jid=`"$script_dir"/filter_observe_array.sh  "$params_file" "$job_dir" "$step_index" helper "$num_jobs" "$input_dep"`
    dep="$dep:$jid"
elif [ "$use_array" = "no_array" ]; then
    for i in `seq $num_jobs`; do
        jid=`"$script_dir"/filter_observe.sh  "$params_file" "$job_dir" "$step_index" helper-$i-$num_jobs "$input_dep"`
        dep="$dep:$jid"
    done
else
...
jid_final=`"$script_dir"/filter_observe.sh  "$params_file" "$job_dir" "$step_index" closer "$dep"`
echo "$jid_final"
