...
#SBATCH --job-name=fig-seismic
#SBATCH -o "$job_dir"/slurm/slurm-out-fig-seismic-%j.txt
#SBATCH --time=4:00:00
#SBATCH -p cpu
#SBATCH --gres=gpu:0
#SBATCH --ntasks=1
...
/usr/bin/time --verbose srun julia scripts/filter_figures_seismic.jl "$params_file" "$job_dir"
EOT
