...
if "EnsembleKalmanFilters" in keys(Pkg.project().dependencies)
    using EnsembleKalmanFilters: EnsembleKalmanFilter
end
if "EnsembleNormalizingFlowFilters" in keys(Pkg.project().dependencies)
    using EnsembleNormalizingFlowFilters: EnsembleNormFlowFilter
end
if "EnsembleJustObsFilters" in keys(Pkg.project().dependencies)
    using EnsembleJustObsFilters: EnsembleJustObsFilter
end
...
# function EnsembleFilters.load_filter(params::Dict, filepath)
#     algorithm = params["filter"]["algorithm"]
#     if algorithm == "enkf"
#         return load_filter(filepath, Val(EnsembleKalmanFilter))
#     elseif algorithm == "no_observations"
#         return load_filter(filepath, Val(EnsembleNoObsFilter))
#     elseif algorithm == "just_observations"
#         return load_filter(filepath, Val(EnsembleJustObsFilter))
#     elseif algorithm == "ekf"
#         return load_filter(filepath, Val(ExtendedKalmanFilter))
#     elseif algorithm == "nf"
#         return load_filter(filepath, Val(EnsembleNormFlowFilter))
#     end
#     error("Unsupported algorithm: $(algorithm)")
# end
